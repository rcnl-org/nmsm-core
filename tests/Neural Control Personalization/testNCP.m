...
% % Muscle Parameters
inputs.maxIsometricForce = [EMGD_data.Fmax, EMGD_data.Fmax, TrunkMuscParam.FMo, TrunkMuscParam.FMo];
...
numMuscles = 148;      inputs.numMuscles = numMuscles;
numLegMuscles = 90;  inputs.numLegMuscles = numLegMuscles;
numTrunkMuscles = 58; inputs.numTrunkMuscles = numTrunkMuscles;
numJoints = 15;        inputs.numJoints = numJoints;
numNodes = 21;         inputs.numNodes = numNodes;
numSynergies = 12;     inputs.numSynergies = numSynergies;
numPoints = 101;          inputs.numPoints = numPoints;
...
momentTrackingWeight = 1; inputs.momentTrackingWeight = momentTrackingWeight;
activationTrackingWeight = 1; inputs.activationTrackingWeight = activationTrackingWeight;
activationMinimizationWeight = 1; inputs.activationMinimizationWeight = activationMinimizationWeight;
momentTrackingAllowableError = 5; inputs.momentTrackingAllowableError = momentTrackingAllowableError; % 5 Nm is the allowable error for moment tracking error
activationTrackingAllowableError = 0.01; inputs.activationTrackingAllowableError = activationTrackingAllowableError;       % 0.01 is the allowable error for activation tracking error
activationMinimizationAllowableError = 0.05; inputs.activationMinimizationAllowableError = activationMinimizationAllowableError;             % 0.05 is the allowable error for activation minimization

inputs.actTr_allow_err = 0.05;
savefilename = ['Syn' num2str(numSynergies) '_' num2str(momentTrackingWeight) '_' num2str(activationTrackingWeight) '_' num2str(activationMinimizationWeight) '_' num2str(momentTrackingAllowableError) '_' num2str(activationTrackingAllowableError) '_' num2str(activationMinimizationAllowableError)];

try
    initial_soln = load(savefilename);
catch err
    initial_soln.x = rand(1,2*(numSynergies/2*(numMuscles/2 + numNodes)));
end
...
emgActivation = inputs.emgActivation;
...
tic
if 1
    x = computeNeuralControlOptimization(x0,inputs,[]);
else
    x = x0;
end
toc
...
inputs.NCPtimePercent = linspace(0,100,numPoints)';
...
numJoints = inputs.numJoints;
numMuscles = inputs.numMuscles;
long2short_idx = inputs.long2short_idx;

MA = zeros(numPoints,numMuscles,numJoints);
for i=1:numJoints
...
%% EMG activations
numPoints = inputs.numPoints;
emgActivation = [EMGD_data.a((trial_no-1)*numPoints+1:trial_no*numPoints,:),act_leftleg.activation_L.case_3];
