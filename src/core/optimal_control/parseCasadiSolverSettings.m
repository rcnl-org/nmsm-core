...
% Used for converting FD cost terms to AD cost terms with FD constraints
inputs.convertFdCostTerms = getBooleanLogicFromField( ...
    getFieldByNameOrAlternate(settingsTree, ...
    "convert_finite_difference_cost_terms", true));
inputs.convertedCostControlBound = getDoubleFromField( ...
    getFieldByNameOrAlternate(settingsTree, ...
    "converted_fd_cost_value_bound_scale_factor", 10));
inputs.convertedCostConstraintBound = getDoubleFromField( ...
    getFieldByNameOrAlternate(settingsTree, ...
    "converted_fd_cost_constraint_bound_scale_factor", 0.01));
