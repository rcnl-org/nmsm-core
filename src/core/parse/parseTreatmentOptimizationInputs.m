...
% Convert FD cost terms to constraints for CasADi
if strcmp(inputs.solverType, 'casadi') && inputs.convertFdCostTerms
    [inputs.costTerms, inputs.path, inputs.terminal, ...
        inputs.numConvertedContinuousCosts, ...
        inputs.numConvertedDiscreteCosts, ...
        inputs.convertedControlBounds, ...
        inputs.convertedParameterBounds] = ...
        convertFdCostTermsToConstraints(inputs);
end
end
...
function [costTerms, path, terminal, numConvertedContinuousCosts, ...
    numConvertedDiscreteCosts, convertedControlBounds, ...
    convertedParameterBounds] = convertFdCostTermsToConstraints(inputs)
costTerms = {};
costTermsToAppend = {};
path = inputs.path;
terminal = inputs.terminal;
numConvertedContinuousCosts = 0;
numConvertedDiscreteCosts = 0;
convertedControlBounds = [];
convertedParameterBounds = [];
[continuousHandles, continuousTermNames, continuousAdSupport] = generateCostTermStruct( ...
    'continuous', inputs.controllerTypes, inputs.toolName);
[discreteHandles, discreteTermNames, discreteAdSupport] = generateCostTermStruct( ...
    'discrete', inputs.controllerTypes, inputs.toolName);
for i = 1 : length(inputs.costTerms)
    if strcmpi(inputs.costTerms{i}.type, "user_defined")
        costTerms{end+1} = inputs.costTerms{i};
    elseif any(strcmpi(inputs.costTerms{i}.type, continuousTermNames))
        if continuousAdSupport(strcmpi(inputs.costTerms{i}.type, ...
            continuousTermNames))
            costTerms{end+1} = inputs.costTerms{i};
        else
            numConvertedContinuousCosts = numConvertedContinuousCosts + 1;
            convertedControlBounds(end+1) = ...
                inputs.convertedCostControlBound * ...
                inputs.costTerms{i}.maxAllowableError;
            costTermsToAppend{end+1} = struct( ...
                'type', 'converted_fd_continous_cost_minimization', ...
                'isEnabled', inputs.costTerms{i}.isEnabled, ...
                'maxAllowableError', ...
                inputs.costTerms{i}.maxAllowableError, ...
                'convertedCostControlIndex', numConvertedContinuousCosts);
            path{end+1} = struct( ...
                'type', 'converted_fd_continuous_cost_path', ...
                'isEnabled', inputs.costTerms{i}.isEnabled, ...
                'maxError', inputs.convertedCostConstraintBound, ...
                'minError', -1 * inputs.convertedCostConstraintBound, ...
                'convertedCostTerm', inputs.costTerms{i}, ...
                'costHandle', ...
                continuousHandles.(inputs.costTerms{i}.type), ...
                'convertedCostControlIndex', numConvertedContinuousCosts);
            if isfield(inputs.costTerms{i}, 'time_ranges')
                costTermsToAppend{end}.time_ranges = ...
                    inputs.costTerms{i}.time_ranges;
                path{end}.time_ranges = inputs.costTerms{i}.time_ranges;
            end
        end
    elseif any(strcmpi(inputs.costTerms{i}.type, discreteTermNames))
        if discreteAdSupport(strcmpi(inputs.costTerms{i}.type, ...
            discreteTermNames))
            costTerms{end+1} = inputs.costTerms{i};
        else
            numConvertedDiscreteCosts = numConvertedDiscreteCosts + 1;
            convertedParameterBounds(end+1) = ...
                inputs.convertedCostControlBound * ...
                inputs.costTerms{i}.maxAllowableError;
            costTermsToAppend{end+1} = struct( ...
                'type', 'converted_fd_discrete_cost_minimization', ...
                'isEnabled', inputs.costTerms{i}.isEnabled, ...
                'maxAllowableError', ...
                inputs.costTerms{i}.maxAllowableError, ...
                'convertedCostParameterIndex', numConvertedDiscreteCosts);
            terminal{end+1} = struct( ...
                'type', 'converted_fd_discrete_cost_terminal', ...
                'isEnabled', inputs.costTerms{i}.isEnabled, ...
                'maxError', inputs.convertedCostConstraintBound, ...
                'minError', -1 * inputs.convertedCostConstraintBound, ...
                'convertedCostTerm', inputs.costTerms{i}, ...
                'costHandle', ...
                discreteHandles.(inputs.costTerms{i}.type), ...
                'convertedCostParameterIndex', numConvertedDiscreteCosts);
            if isfield(inputs.costTerms{i}, 'time_ranges')
                costTermsToAppend{end}.time_ranges = ...
                    inputs.costTerms{i}.time_ranges;
                terminal{end}.time_ranges = ...
                    inputs.costTerms{i}.time_ranges;
            end
        end
    else
        error(inputs.costTerms{i}.type + ...
            " is not a valid cost term type.");
    end
end
if ~isempty(costTermsToAppend)
    costTerms(end+1 : end+length(costTermsToAppend)) = ...
        costTermsToAppend;
end
end
