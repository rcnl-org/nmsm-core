...
try
    ...
    [~, presentStatesIndices, indices] = intersect( ...
        inputs.statesCoordinateNames, ...
        inputs.initialStatesLabels, 'stable');
    [addedStates, addedStatesIndices] = setdiff( ...
        inputs.statesCoordinateNames, ...
        inputs.initialStatesLabels, 'stable');
    initialStatesLabels = string([]);
    if inputs.useJerk
        numOriginalStates = length(inputs.initialStatesLabels) / 3;
    else
        numOriginalStates = length(inputs.initialStatesLabels) / 2;
    end
    numNewStates = length(inputs.statesCoordinateNames);
    initialStatesLabels(presentStatesIndices) = ...
        inputs.initialStatesLabels(indices);
    initialStatesLabels(presentStatesIndices + numNewStates) = ...
        inputs.initialStatesLabels(indices + numOriginalStates);
    if inputs.useJerk
        initialStatesLabels(numNewStates * 2 + presentStatesIndices) = ...
            inputs.initialStatesLabels(indices + numOriginalStates * 2);
    end
    inputs.initialStatesLabels = initialStatesLabels;
    initialStates(:, presentStatesIndices) = ...
        inputs.initialStates(:, indices);
    initialStates(:, presentStatesIndices + numNewStates) ...
        = inputs.initialStates(:, indices + numOriginalStates);
    if inputs.useJerk
        initialStates(:, presentStatesIndices + numNewStates * 2) ...
            = inputs.initialStates(:, indices + numOriginalStates * 2);
    end
    inputs.initialStates = initialStates;
    if ~isempty(addedStatesIndices)
        [~, ~, indices] = intersect( ...
            addedStates, ...
            inputs.coordinateNamesStrings, 'stable');
        addedCoordinateSplines = makeGcvSplineSet( ...
            inputs.experimentalTime, ...
            inputs.experimentalJointAngles(:, indices), ...
            inputs.coordinateNamesStrings(indices));
        inputs.initialStatesLabels(addedStatesIndices) = ...
            inputs.coordinateNamesStrings(indices);
        inputs.initialStates(:, addedStatesIndices) = ...
            evaluateGcvSplines(addedCoordinateSplines, addedStates, ...
            inputs.initialTime);
        inputs.initialStatesLabels(addedStatesIndices + numNewStates) = ...
            inputs.coordinateNamesStrings(indices) + "_u";
        inputs.initialStates(:, addedStatesIndices + numNewStates) = ...
            evaluateGcvSplines(addedCoordinateSplines, addedStates, ...
            inputs.initialTime, 1);
        if inputs.useJerk
            inputs.initialStatesLabels(addedStatesIndices + ...
                numNewStates * 2) = ...
                inputs.coordinateNamesStrings(indices) + "_dudt";
            inputs.initialStates(:, addedStatesIndices + ...
                numNewStates * 2) = ...
                evaluateGcvSplines(addedCoordinateSplines, addedStates, ...
...
try
    ...
    [~, presentAccelerationsIndices, indices] = intersect( ...
        inputs.statesCoordinateNames, ...
        inputs.initialAccelerationsLabels, 'stable');
    [addedAccelerations, addedAccelerationsIndices] = setdiff( ...
        inputs.statesCoordinateNames, ...
        inputs.initialAccelerationsLabels, 'stable');
    initialAccelerations(:, presentAccelerationsIndices) = ...
        inputs.initialAccelerations(:, indices);
    if ~isempty(addedAccelerationsIndices)
        initialAccelerations(:, addedAccelerationsIndices) = ...
            evaluateGcvSplines(addedCoordinateSplines, addedAccelerations, ...
            inputs.initialTime, 2);
    end
    initialAccelerationsLabels(presentAccelerationsIndices) = ...
        inputs.initialAccelerationsLabels(indices);
    if ~isempty(addedAccelerationsIndices)
        initialAccelerationsLabels(addedAccelerationsIndices) = ...
            inputs.statesCoordinateNames(addedAccelerationsIndices);
    end
    [inputs.initialAccelerations, inputs.initialAccelerationsLabels] = ...
        reorderInitialValues(initialAccelerations, ...
        initialAccelerationsLabels, ...
        inputs.statesCoordinateNames, "<states_coordinate_list> does " + ...
        "not match initial accelerations.");
    if inputs.useJerk
        [inputs.initialJerks, inputs.initialJerkLabels] = ...
            parseTrialData(inputs.initialGuessDirectory, ...
            strcat(inputs.trialName, "_jerks"), inputs.model);
        [~, presentJerksIndices, indices] = intersect( ...
            inputs.statesCoordinateNames, ...
            inputs.initialJerkLabels, 'stable');
        [addedJerks, addedJerksIndices] = setdiff( ...
            inputs.statesCoordinateNames, ...
            inputs.initialAccelerationsLabels, 'stable');
        initialJerks(:, presentJerksIndices) = ...
            inputs.initialJerks(:, indices);
        if ~isempty(addedJerksIndices)
            initialJerks(:, addedJerksIndices) = ...
                evaluateGcvSplines(addedCoordinateSplines, addedJerks, ...
                inputs.initialTime, 3);
        end
        initialJerkLabels(presentJerksIndices) = ...
            inputs.initialJerkLabels(indices);
        if ~isempty(addedJerksIndices)
            initialJerkLabels(addedJerksIndices) = ...
                inputs.statesCoordinateNames(addedJerksIndices);
        end
        inputs.initialJerks = reorderInitialValues(initialJerks, ...
            initialJerkLabels, inputs.statesCoordinateNames, ...
            "<states_coordinate_list> does not match initial jerks.");
        inputs.initialJerkLabels = initialJerkLabels;
    end
catch; end
try
    ...
    [~, presentTorquesIndices, indices] = intersect( ...
        inputs.torqueControllerCoordinateNames, ...
        inputs.initialTorqueControlsLabels, 'stable');
    [addedTorques, addedTorquesIndices] = setdiff( ...
        inputs.torqueControllerCoordinateNames, ...
        inputs.initialTorqueControlsLabels, 'stable');
    initialTorqueControls(:, presentTorquesIndices) = ...
        inputs.initialTorqueControls(:, indices);
    if ~isempty(addedTorquesIndices)
        [~, ~, splineIndices] = intersect( ...
            addedTorques, ...
            inputs.inverseDynamicsMomentLabels, 'stable');
        addedTorqueSplines = makeGcvSplineSet( ...
            inputs.experimentalTime, ...
            inputs.experimentalJointMoments(:, splineIndices), ...
            inputs.inverseDynamicsMomentLabels(splineIndices));
        initialTorqueControls(:, addedTorquesIndices) = ...
            evaluateGcvSplines(addedTorqueSplines, addedTorques, ...
            inputs.initialTime);
    end
    initialTorqueControlsLabels(presentTorquesIndices) = ...
        inputs.initialTorqueControlsLabels(indices);
    if ~isempty(addedTorquesIndices)
        initialTorqueControlsLabels(addedTorquesIndices) = ...
            inputs.torqueControllerCoordinateNames(addedTorquesIndices);
    end
    [inputs.initialTorqueControls, inputs.initialTorqueControlsLabels] = ...
        reorderInitialValues(initialTorqueControls, ...
        initialTorqueControlsLabels, ...
    ...
    if inputs.useControlDynamicsFilter
        [inputs.initialTorqueControlDerivatives, labels] = ...
            parseTrialData(inputs.initialGuessDirectory, ...
            strcat(inputs.trialName, "_torqueControlDerivatives"), inputs.model);
        [~, presentTorquesIndices, indices] = intersect( ...
            inputs.torqueControllerCoordinateNames, ...
            labels, 'stable');
        [~, addedTorquesIndices] = setdiff( ...
            inputs.torqueControllerCoordinateNames, ...
            labels, 'stable');
        initialTorqueControlDerivatives(:, presentTorquesIndices) = ...
            inputs.initialTorqueControlDerivatives(:, indices);
        if ~isempty(addedTorquesIndices)
            initialTorqueControlDerivatives(:, addedTorquesIndices) = ...
                evaluateGcvSplines(addedTorqueSplines, addedTorques, ...
                inputs.initialTime, 1);
        end
        inputs.initialTorqueControlDerivatives = ...
            reorderInitialValues(initialTorqueControlDerivatives, ...
            labels, ...
            inputs.torqueControllerCoordinateNames, "Torque control derivative" + ...
            "coordinate list does not match initial guess.");
    end
catch;end
if inputs.controllerTypes(2)
    ...
    try
        [inputs.initialSynergyControlDerivatives, labels] = ...
            parseTrialData(inputs.initialGuessDirectory, ...
            strcat(inputs.trialName, "_synergyCommandDerivatives"), inputs.model);
        inputs.initialSynergyControlDerivatives = ...
            reorderInitialValues(inputs.initialSynergyControlDerivatives, ...
            labels, ...
            inputs.initialSynergyControlsLabels, "Synergy control derivative" + ...
            "coordinate list does not match initial guess.");
    catch;end
end
try
    ...
    if inputs.useControlDynamicsFilter
        [inputs.initialMuscleControlDerivatives, labels] = ...
            parseTrialData(inputs.initialGuessDirectory, ...
            strcat(inputs.trialName, "_muscleControlDerivatives"), inputs.model);
        inputs.initialMuscleControlDerivatives = ...
            reorderInitialValues(inputs.initialMuscleControlDerivatives, ...
            labels, ...
            inputs.initialMuscleControlsLabels, "Muscle control derivative" + ...
            "coordinate list does not match initial guess.");
    end
catch;end
try
    ...
    if inputs.useControlDynamicsFilter
        [inputs.initialUserDefinedControlDerivatives, labels] = ...
            parseTrialData(inputs.initialGuessDirectory, ...
            strcat(inputs.trialName, "_userDefinedControlDerivatives"), inputs.model);
        inputs.initialUserDefinedControlDerivatives = ...
            reorderInitialValues(inputs.initialUserDefinedControlDerivatives, ...
            labels, ...
            inputs.initialUserDefinedControlLabels, "User defined control derivative" + ...
            "coordinate list does not match initial guess.");
    end
catch;end
end
