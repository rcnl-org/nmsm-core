...
for i=1:length(groupNames)
    ...
    for j=0:model.getForceSet().getGroup(groupNames(i)).getMembers().getSize()-1
        count = 1;
        found = false;
        memberName = model.getForceSet().getGroup( ...
        ...
        if ~found
            throw(MException('', '%s', "Muscle group """ + ...
                string(groupNames(i)) + """ member """ + ...
                string(memberName) + """ was not found among the " + ...
                "model's muscles."))
        end
        group(end+1) = count;
    end
