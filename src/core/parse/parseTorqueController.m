...
inputs.torqueControllerCoordinateNames = parseSpaceSeparatedList(tree, ...
    "coordinate_list");
inputs.maxTorqueControlsMultiple = double(split(string( ...
    parseElementTextByNameOrAlternate(tree, ...
    'torque_controls_range_scale_factor', 1))))';
inputs.maxTorqueControlsMinRange = double(split(string( ...
    parseElementTextByNameOrAlternate(tree, ...
    'torque_controls_minimum_range', 0))))';
inputs.torqueControlDerivativesMultiple = double(split(string( ...
    parseElementTextByNameOrAlternate(tree, ...
    'torque_control_derivatives_range_scale_factor', 1))))';
inputs.torqueControlDerivativesMinRange = double(split(string( ...
    parseElementTextByNameOrAlternate(tree, ...
    'torque_control_derivatives_minimum_range', 0))))';
end
