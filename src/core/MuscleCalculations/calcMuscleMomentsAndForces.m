...
onesCol = ones(size(inputData.muscleTendonLength, 1:2));
passiveForce = onesCol .* (permute(inputData.maxIsometricForce, [1 3 2])  .* ...
    permute(cos(inputData.pennationAngle), [1 3 2]) ) .* ...
    passiveForceLengthCurve(lMtilda);
% equation 1 from Meyer 2017
muscleForce = onesCol .* (permute(inputData.maxIsometricForce, [1 3 2]) .* ...
    permute(cos(inputData.pennationAngle), [1 3 2])) .* ...
