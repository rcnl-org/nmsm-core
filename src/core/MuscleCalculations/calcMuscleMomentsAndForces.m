...
function [passiveForce, muscleForce, muscleMoments, modelMoments] = ...
    calcMuscleMomentsAndForces(experimentalData, muscleActivations, lMtilda, ...
    normalizedFiberVelocity)

% Preallocation of Memory
muscleMoments = zeros([size(experimentalData.muscleTendonLength), size(experimentalData.momentArms, 4)]); 
onesCol = ones(size(experimentalData.muscleTendonLength, 1:2));
passiveForce = onesCol .* (permute(experimentalData.maxIsometricForce, [1 3 2])  .* ...
    permute(cos(experimentalData.pennationAngle), [1 3 2]) ) .* ...
    passiveForceLengthCurve(lMtilda);
% equation 1 from Meyer 2017
muscleForce = onesCol .* (permute(experimentalData.maxIsometricForce, [1 3 2]) .* ...
    permute(cos(experimentalData.pennationAngle), [1 3 2])) .* ...
    muscleActivations .* activeForceLengthCurve(lMtilda) .* ...
    forceVelocityCurve(normalizedFiberVelocity) + passiveForce;
for i=1:size(experimentalData.momentArms, 4)
    muscleMoments(:, :, :, i) = experimentalData.momentArms(:, :, :, i) .* ...
        muscleForce;
end
