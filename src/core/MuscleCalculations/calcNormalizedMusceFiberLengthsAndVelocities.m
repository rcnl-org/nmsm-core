...
lMtilda = (hillTypeParams.lMt - onesCol .* findCorrectMtpValues(6, ...
    valuesStruct)) ./ (onesCol .* (findCorrectMtpValues(5, ...
    valuesStruct) .* cos(hillTypeParams.pennationAngle)));
% Normalized muscle fiber velocity, equation 3 from Meyer 2017
vMtilda = hillTypeParams.vMt ./ (hillTypeParams.vMaxFactor * onesCol .* ...
    (findCorrectMtpValues(5, valuesStruct) .* cos( ...
    hillTypeParams.pennationAngle)));
end
