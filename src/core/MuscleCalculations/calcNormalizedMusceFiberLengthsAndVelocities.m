...
scaledOptimalFiberLength = permute(inputData.optimalFiberLength .* ...
    findCorrectMtpValues(6, valuesStruct), [1 3 2]);
scaledTendonSlackLength = permute(inputData.tendonSlackLength .* ...
    findCorrectMtpValues(5, valuesStruct), [1 3 2]);
onesCol = ones(size(inputData.muscleTendonLength, 1), ...
    size(inputData.muscleTendonLength, 2));
% Normalized muscle fiber length, equation 2 from Meyer 2017
lMtilda = (inputData.muscleTendonLength - onesCol .* ...
    scaledTendonSlackLength) ./ (onesCol .* ...
    (scaledOptimalFiberLength .* permute(cos(inputData.pennationAngle), ...
    [1 3 2])));
% Normalized muscle fiber velocity, equation 3 from Meyer 2017
vMtilda = inputData.muscleTendonVelocity ./ ...
    (inputData.vMaxFactor * onesCol .* (scaledOptimalFiberLength .* ...
