...
% plot joint moments
plotResultsWithComparison(solution.inverseDynamicMoments(:, ...
    inputs.trackedInverseDynamicMomentsIndex), values.time, ...
    inputs.experimentalJointMoments(:, ...
    inputs.trackedInverseDynamicMomentsIndex), inputs.experimentalTime, ...
    strcat(inputs.inverseDynamicMomentLabels(:, ...
    inputs.trackedInverseDynamicMomentsIndex), ' [Nm]'), ["Joint" "Moments"]);
