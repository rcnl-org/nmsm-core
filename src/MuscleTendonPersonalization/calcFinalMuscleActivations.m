...
function muscleActivations = calcFinalMuscleActivations(results, inputData)

muscleExcitations = calcMuscleExcitations(inputData.timeEMG, ...
    inputData.emgSplines, findCorrectMtpValues(1, results), ...
    findCorrectMtpValues(4, results));
neuralActivations = calcNeuralActivations(muscleExcitations, ...
    findCorrectMtpValues(2, results), inputData.timeEMG, inputData.nPad);
muscleActivations = calcMuscleActivations(findCorrectMtpValues(3, ...   
    results), neuralActivations);
end