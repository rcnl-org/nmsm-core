...
costWeight = valueOrAlternate(params, ...
    "normalizedFiberLengthGroupedSimiliarityCostWeight", 1);
errorCenter = valueOrAlternate(params, ...
    "normalizedFiberLengthGroupedSimiliarityErrorCenter", 0);
maximumAllowableError = valueOrAlternate(params, ...
    "normalizedFiberLengthGroupedSimiliarityMaximumAllowableError", 0.05);
index = 1;
for i = 1:length(experimentalData.normalizedFiberLengthGroups)
    muscleGroupGroup = experimentalData.normalizedFiberLengthGroups{i};
    normalizedFiberLengthMagnitudeDeviation(:, index : index + size(...
        muscleGroupGroup, 2) - 1) = calcMeanDeviations( ...
        modeledValues.normalizedFiberLength(:, muscleGroupGroup, :), ...
        experimentalData.normalizedFiberLength(:, muscleGroupGroup, :));
    normalizedFiberLengthShapeDeviation(:, index : index + size(...
        muscleGroupGroup, 2) - 1, :) = calcMeanShapeDeviations( ...
        modeledValues.normalizedFiberLength(:, muscleGroupGroup, :));
    index = index + size(muscleGroupGroup, 2);
end
