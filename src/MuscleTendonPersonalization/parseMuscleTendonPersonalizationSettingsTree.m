...
function inputs = parseMtpInitialGuess(tree, inputs)
inputs.muscleSpecificElectromechanicalDelays = ...
    strcmp("true", parseElementTextByNameOrAlternate(tree, ...
    "muscle_specific_electromechanical_delays", "true"));
inputs.electromechanicalDelayInitialGuess = str2double( ...
    parseElementTextByNameOrAlternate(tree, ...
    "electromechanical_delay_initial_guess", '0.5'));
inputs.activationTimeConstantInitialGuess = str2double( ...
    parseElementTextByNameOrAlternate(tree, ...
    "activation_time_constant_initial_guess", '1.5'));
inputs.activationNonlinearityInitialGuess = str2double( ...
    parseElementTextByNameOrAlternate(tree, ...
    "activation_nonlinearity_initial_guess", '0.05'));
inputs.emgScaleFactorInitialGuess = str2double( ...
    parseElementTextByNameOrAlternate(tree, ...
    "emg_scale_factor_initial_guess", '0.5'));
inputs.optimalFiberLengthScaleFactorInitialGuess = str2double( ...
    parseElementTextByNameOrAlternate(tree, ...
    "optimal_fiber_length_scale_factor_initial_guess", '1'));
inputs.tendonSlackLengthScaleFactorInitialGuess = str2double( ...
    parseElementTextByNameOrAlternate(tree, ...
    "tendon_slack_length_scale_factor_initial_guess", '1'));
