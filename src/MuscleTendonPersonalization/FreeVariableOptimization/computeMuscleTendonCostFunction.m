...
neuralActivations = calcNeuralActivations(muscleExcitations, ...
    findCorrectMtpValues(2, valuesStruct), data.emgTime, ...
    data.numPaddingFrames);
...
costs = calcAllDeviationPenaltyCosts(valuesStruct, params, data, ...
    passiveForce, costs);
costs = calcLmTildaCurveChangesCost(lMtilda, ...
    params.lMtildaExperimental, params.lmtildaPairs, ...
