...
%   experimentalData.SynXCommands - synergy excitations extracted from measured muscle
%                  excitations for SynX
%   experimentalData.ResExtCommands - synergy excitations extracted from measured muscle excitations 
...
unmeasuredEmgSignals = getUnmeasuredMuscleExcitations(params, ...
    experimentalData.emgData, experimentalData.SynXCommands, ...
    synergyExtrapolationVariables(1 : experimentalData.nSynxVariables));
% Reconstruct residual muscle excitations for measured muscles
residualExcitations = getResidualMuscleExcitation(params, ...
    experimentalData.ResExtCommands, synergyExtrapolationVariables( ...
    experimentalData.nSynxVariables + 1 : end));
...
function UnmeasuredExcitations = getUnmeasuredMuscleExcitations(params, ...
    emgData, synxCommands, SynXVarsVector)
...
if strcmpi(params.matrixFactorizationMethod,'PCA')
    SynxVars = reshape(SynXVarsVector',(params.numberOfSynergies + 1) * ...
        length(params.synergyCategorizationOfTrials), ...
        size(params.missingEmgChannelPairs, 2))'; 
    for i = 1 : length(params.synergyCategorizationOfTrials)
        UnmeasuredExcitations(:, j, i) = synxCommands{i} * SynxVars(j, ...
...
elseif strcmp(params.matrixFactorizationMethod,'NMF')
    ...
    for i = 1 : length(params.synergyCategorizationOfTrials)
        ...
        UnmeasuredExcitations(:, j, i) = synxCommands{i} * ...
...
function residualExcitations = getResidualMuscleExcitation(params, ...
    ResExtCommands, ResVarsVector)

if strcmpi(params.matrixFactorizationMethod,'PCA')
    ResVar = reshape(ResVarsVector', (params.numberOfSynergies + 1), ...
        size(params.missingEmgChannelPairs, 2), ...
        size(params.residualCategorizationOfTrials, 2));
    for i = 1 : size(params.residualCategorizationOfTrials, 2)
        residualExcitations{i}= ResExtCommands{i} * ResVar(1 : ...
            params.numberOfSynergies, :, i) + ResVar( ...
            params.numberOfSynergies + 1, : , i);
    end
elseif strcmp(params.matrixFactorizationMethod,'NMF')
    ResVar = reshape(ResVarsVector', params.numberOfSynergies, ...
        size(params.missingEmgChannelPairs, 2), ...
        size(params.residualCategorizationOfTrials, 2));
    for i = 1 : size(params.residualCategorizationOfTrials, 2)
        residualExcitations{i}= ResExtCommands{i} * ResVar(1 : ...
