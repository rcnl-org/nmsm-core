...
%   size(params.missingEmgChannelGroups,2) - number of SynX EMG channels in total - double
%   size(params.missingEmgChannelGroups,2) - number of measured EMG channels in total - double
...
muscleExcitationsNoTDelay = distributeResidualExcitations(emgData, ...
    params.currentEmgChannelGroups, residualExcitations);
% Create EMG splines
...
if strcmpi(params.matrixFactorizationMethod,'PCA')
    ResVar = reshape(residualWeights', (params.numberOfSynergies + 1), ...
        size(params.currentEmgChannelGroups, 2), ...
