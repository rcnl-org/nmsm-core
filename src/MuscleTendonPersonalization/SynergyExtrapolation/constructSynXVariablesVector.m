...
function [SynXVariableVector,nSynxVariables,nResVariables] = ...
    constructSynXVariablesVector(params, numberOfTrials, ...
    numberOfMeasuredEmgChannels, numberOfUnmeasuredEmgChannels)

% Construct variable vector for SynX
SynxVars = getSizeMatrix(params.synergyExtrapolationCategorization, ...
...
ResVars = getSizeMatrix(params.residualCategorization, ...
    params.matrixFactorizationMethod, params.numberOfSynergies, ...
    numberOfMeasuredEmgChannels, length(params.taskNames), numberOfTrials);
SynXVariableVector= [SynxVars(:)', ResVars(:)'];
nSynxVariables = numel(SynxVars);
nResVariables = numel(ResVars);
end
