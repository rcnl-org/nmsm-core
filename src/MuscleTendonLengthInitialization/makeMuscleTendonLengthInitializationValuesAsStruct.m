...
function [values, experimentalData] = ...
    makeMuscleTendonLengthInitializationValuesAsStruct(parameterChange, ...
    experimentalData)
if experimentalData.useAbsoluteLengths
    experimentalData.optimalFiberLength = parameterChange(:, 1 : ...
        length(experimentalData.muscleNames));
    experimentalData.tendonSlackLength = ...
        parameterChange(:, length(experimentalData.muscleNames) + 1 : ...
        2 * length(experimentalData.muscleNames));
    values.optimalFiberLengthScaleFactors = ...
        ones(1, length(experimentalData.muscleNames));
