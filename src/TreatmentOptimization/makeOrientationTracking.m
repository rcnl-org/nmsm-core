...
for i = 1:length(inputs.costTerms)
    ...
    if costTerm.isEnabled && any(strcmp(costTerm.type, orientationCostTerms))
...
for i = 1:length(inputs.path)
    ...
    if constraintTerm.isEnabled && any(strcmp(constraintTerm.type, orientationConstraintTerms))
        names(end + 1) = convertCharsToStrings(inputs.model.getBodySet().get( ...
...
if ~isempty(inputs.trackedOrientationNames)
    ...
    [~, ~, ~, ~, bodyOrientations, ~] = inverseDynamics( ...
        ...
        inputs.trackedOrientationIndices, [], 0, 0, 1, 0, inputs.osimVersion);
