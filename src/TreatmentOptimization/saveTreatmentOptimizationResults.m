...
ikDataSpline = makeGcvSplineSet(values.time, values.positions, ...
    inputs.coordinateNames);

velocities = evaluateGcvSplines(ikDataSpline, ...
    inputs.coordinateNames, time, 1);
accelerations = evaluateGcvSplines(ikDataSpline, ...
    inputs.coordinateNames, time, 2);
...
% [time, positions] = splineToEvenlySpaced(values.time, ...
    % inputs., length(inputs.experimentalTime));
