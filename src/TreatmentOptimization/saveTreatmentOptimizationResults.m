...
for i = 1:length(inputs.contactSurfaces)
    ...
    % The model produces one wrench per contact surface. It is written back
    % out under the columns of whichever force plate carried the surface at
    % that time, so results keep the layout of the input GRF file.
    activePlate = findActiveForcePlate(inputs.contactSurfaces{i}, ...
        values.time);
    numForcePlates = numel(inputs.contactSurfaces{i}.forceColumns) / 3;
    for plate = 1:numForcePlates
        columns = 3 * (plate - 1) + (1:3);
        groundContactLabels = cat(2, groundContactLabels, ...
            [inputs.contactSurfaces{i}.forceColumns(columns), ...
            inputs.contactSurfaces{i}.electricalCenterColumns(columns), ...
            inputs.contactSurfaces{i}.momentColumns(columns)]);
        onThisPlate = activePlate == plate;
        groundContactData = [groundContactData, ...
            solution.groundReactionsLab.forces{i} .* onThisPlate, ...
            midfootSuperiorLocation, ...
            solution.groundReactionsLab.moments{i} .* onThisPlate];
    end
end
...
columnLabels = strings(1, 0);
dataToSave = zeros(size(inputs.experimentalTime, 1), 0);
for surface = 1 : numel(inputs.contactSurfaces)
    contactSurface = inputs.contactSurfaces{surface};
    activePlate = findActiveForcePlate(contactSurface, ...
        inputs.experimentalTime);
    numForcePlates = numel(contactSurface.forceColumns) / 3;
    for plate = 1 : numForcePlates
        columns = 3 * (plate - 1) + (1:3);
        plateElectricalCenter = ...
            findPlateElectricalCenter(contactSurface, plate);
        % setupGroundContact referenced these moments to the midfoot
        % superior point. Move them back onto this plate's electrical
        % center so they mean what the column labels say.
        moments = contactSurface.experimentalGroundReactionMoments;
        if isfield(contactSurface, 'midfootSuperiorLocation')
            moments = transferMoments( ...
                contactSurface.midfootSuperiorLocation, ...
                plateElectricalCenter, moments, ...
                contactSurface.experimentalGroundReactionForces);
        end
        onThisPlate = activePlate == plate;
        columnLabels = [columnLabels, ...
            contactSurface.forceColumns(columns), ...
            contactSurface.momentColumns(columns), ...
            contactSurface.electricalCenterColumns(columns)];
        dataToSave = [dataToSave, ...
            contactSurface.experimentalGroundReactionForces .* onThisPlate, ...
            moments .* onThisPlate, ...
            plateElectricalCenter];
    end
end
...
% Which force plate carries a surface at each of the given times, from the
% contact windows recorded when the GRF file was parsed.
function activePlate = findActiveForcePlate(contactSurface, time)
if ~isfield(contactSurface, 'forcePlates') || ...
        numel(contactSurface.forcePlates.windowStartTimes) < 2
    activePlate = ones(length(time), 1);
    return
end
time = time(:);
normalizedTime = (time - time(1)) / (time(end) - time(1));
activePlate = interp1(contactSurface.forcePlates.windowStartTimes, ...
    contactSurface.forcePlates.windowPlates, normalizedTime, ...
    'previous', 'extrap');
end

function plateElectricalCenter = findPlateElectricalCenter( ...
    contactSurface, plate)
if isfield(contactSurface, 'forcePlates')
    plateElectricalCenter = ...
        contactSurface.forcePlates.electricalCenter(:, :, plate);
else
    plateElectricalCenter = contactSurface.electricalCenter;
end
end

function saveMiscellaneousAnalysisFiles(solution, inputs, values)
    if ~exist(fullfile(inputs.resultsDirectory, "Analysis"), "dir")
        mkdir(fullfile(inputs.resultsDirectory, "Analysis"))
    end
    %% Save modeled positions, velocities, and accelerations
    [time, positions] = splineToEvenlySpaced(values.time, ...
        values.positions, length(inputs.experimentalTime));
    writeToSto(inputs.coordinateNames, time, positions, ...
        fullfile(inputs.resultsDirectory, "Analysis", ...
        strcat(inputs.trialName, "_modeledPositions.sto")))
    
    [~, velocities] = splineToEvenlySpaced(values.time, ...
        values.velocities, length(inputs.experimentalTime));
    writeToSto(inputs.coordinateNames, time, velocities, ...
        fullfile(inputs.resultsDirectory, "Analysis", ...
        strcat(inputs.trialName, "_modeledVelocities.sto")))
    
    [~, accelerations] = splineToEvenlySpaced(values.time, ...
        values.accelerations, length(inputs.experimentalTime));
    writeToSto(inputs.coordinateNames, time, accelerations, ...
        fullfile(inputs.resultsDirectory, "Analysis", ...
        strcat(inputs.trialName, "_modeledAccelerations.sto")))

    %% Save tracked positions, velocities, and accelerations
    trackedPositionsSpline = makeGcvSplineSet(inputs.experimentalTime, ...
        inputs.experimentalJointAngles, inputs.coordinateNames);

    writeToSto(inputs.coordinateNames, time, ...
        inputs.experimentalJointAngles, ...
        fullfile(inputs.resultsDirectory, "Analysis", ...
        strcat(inputs.trialName, "_trackedPositions.sto")))
    trackedVelocities = evaluateGcvSplines(trackedPositionsSpline, ...
        inputs.coordinateNames, inputs.experimentalTime, 1);
    writeToSto(inputs.coordinateNames, time, trackedVelocities, ...
        fullfile(inputs.resultsDirectory, "Analysis", ...
        strcat(inputs.trialName, "_trackedVelocities.sto")))
    trackedAccelerations = evaluateGcvSplines(trackedPositionsSpline, ...
        inputs.coordinateNames, inputs.experimentalTime, 2);
    writeToSto(inputs.coordinateNames, time, trackedAccelerations, ...
        fullfile(inputs.resultsDirectory, "Analysis", ...
        strcat(inputs.trialName, "_trackedAccelerations.sto")))
end