...
saveTrackedInverseKinematics(inputs, inputs.resultsDirectory);
...
[~, velocities] = splineToEvenlySpaced(values.time, values.velocities, ...
    length(inputs.experimentalTime));
writeToSto(inputs.coordinateNames, time, velocities, ...
    fullfile(outputDirectory, "IKData", ...
    strcat(inputs.trialName, "_velocities", ".sto")));
...
function saveTrackedInverseKinematics(inputs, outputDirectory)
if ~exist(fullfile(outputDirectory, "IKDataTracked"), "dir")
    mkdir(fullfile(outputDirectory, "IKDataTracked"))
end
writeToSto(inputs.coordinateNames, inputs.experimentalTime, ...
    inputs.experimentalJointAngles, fullfile(outputDirectory, ...
    "IKDataTracked", strcat(inputs.trialName, ".sto")));

% Save results velocities and accelerations
ikDataSpline = makeGcvSplineSet(inputs.experimentalTime, ...
    inputs.experimentalJointAngles, inputs.coordinateNames);

velocities = evaluateGcvSplines(ikDataSpline, ...
    inputs.coordinateNames, inputs.experimentalTime, 1);
writeToSto(inputs.coordinateNames, inputs.experimentalTime, velocities, ...
    fullfile(outputDirectory, "IKDataTracked", ...
    strcat(inputs.trialName, "_velocities", ".sto")));

accelerations = evaluateGcvSplines(ikDataSpline, ...
    inputs.coordinateNames, inputs.experimentalTime, 2);
writeToSto(inputs.coordinateNames, inputs.experimentalTime, accelerations, ...
    fullfile(outputDirectory, "IKDataTracked", ...
    strcat(inputs.trialName, "_accelerations", ".sto")));
end
