...
%
% Args:
% settingsFileName (string) - filename of xml settings file to parse
%       directories from. If plotting synergy driven & torque driven runs,
%       use the synergy driven settings file.
% 
% resultsDirectories (array of strings) - OPTIONAL, String array of results
...
if isSynergy
    ...
    plotSynergyControls(settingsTree, resultsDirectory, ...
        trackedQuantitiesDirectory, initialGuessDirectory, trialPrefix, modelFileName);
end
end

function plotJointAngles(resultsDirectory, trackedQuantitiesDirectory, trialPrefix, modelFileName)
...
function plotJointVelocities(resultsDirectory, trackedQuantitiesDirectory, trialPrefix, modelFileName)
...
function plotJointLoads(resultsDirectory, trackedQuantitiesDirectory, trialPrefix)
...
function plotMuscleActivations(resultsDirectory, trackedQuantitiesDirectory, initialGuessDirectory, trialPrefix)
...
function plotTorqueControls(resultsDirectory, trackedQuantitiesDirectory, trialPrefix)
torqueIndices = logical([]);
for i = 1 : numel(resultsDirectory)
    if exist(fullfile(resultsDirectory(i), strcat(trialPrefix, "_torqueControls.sto")), "file")
        torqueIndices(i) = 1;
    else
        torqueIndices(i) = 0;
    end
end
if exist(fullfile(trackedQuantitiesDirectory, strcat(trialPrefix, "_torqueControls.sto")), "file")
...
function plotSynergyControls(settingsTree, resultsDirectory, trackedQuantitiesDirectory, initialGuessDirectory, trialPrefix, modelFileName)
