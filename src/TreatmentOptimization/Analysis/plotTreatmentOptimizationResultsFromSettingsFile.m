...
function plotJointVelocities(resultsDirectory, trialPrefix, modelFileName)
% Plot
trackedVelocities = fullfile(resultsDirectory, "Analysis", strcat(trialPrefix, ...
    "_trackedVelocities.sto"));

plotTreatmentOptimizationJointVelocities(modelFileName, ...
    trackedVelocities(1), ...
    fullfile(resultsDirectory, "Analysis", strcat(trialPrefix, ...
    "_modeledVelocities.sto")), useRadians=1, showRmse=1)
end

function plotJointAccelerations(resultsDirectory, trialPrefix, ...
    modelFileName)
trackedAccelerations = fullfile(resultsDirectory, "Analysis", strcat(trialPrefix, ...
    "_trackedAccelerations.sto"));
plotTreatmentOptimizationJointAccelerations(modelFileName, ...
    trackedAccelerations(1), ...
    fullfile(resultsDirectory, "Analysis", strcat(trialPrefix, ...
    "_modeledAccelerations.sto")), useRadians=1, showRmse=1)
if exist(fullfile(resultsDirectory(1), "Analysis"), "dir")
    plotTreatmentOptimizationJointVelocities(modelFileName, ...
        fullfile(resultsDirectory(1), "Analysis", strcat(trialPrefix, ...
        "_trackedVelocities.sto")), ...
        fullfile(resultsDirectory, "Analysis", strcat(trialPrefix, ...
        "_modeledVelocities.sto")), useRadians=1, showRmse=1)
end
