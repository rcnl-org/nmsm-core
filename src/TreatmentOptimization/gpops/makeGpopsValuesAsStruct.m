...
if inputs.useControlDynamicsFilter
    ...
    stateIndex = stateIndex + length(inputs.torqueControllerCoordinateNames);
    values.torqueControlDerivatives = control(:, ...
        controlIndex : ...
        controlIndex - 1 + length(inputs.torqueControllerCoordinateNames));
    controlIndex = controlIndex + length(inputs.torqueControllerCoordinateNames);
else
    if inputs.controllerTypes(4)
...
if inputs.useGroundReactionControls
    values.controlGroundReactions = state(:, stateIndex : ...
        stateIndex - 1 + length( ...
        inputs.initialGroundReactionControlLabels));
end
