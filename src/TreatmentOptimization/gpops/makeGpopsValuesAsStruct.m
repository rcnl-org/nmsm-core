...
if strcmpi(inputs.solverType, 'casadi') && inputs.convertFdCostTerms && ...
        inputs.numConvertedContinuousCosts > 0
    values.convertedFdCostControls = control(:, controlIndex : ...
        controlIndex - 1 + inputs.numConvertedContinuousCosts);
    controlIndex = controlIndex + inputs.numConvertedContinuousCosts;
end
...
if strcmpi(inputs.solverType, 'casadi') && inputs.convertFdCostTerms && ...
    ...
    values.parameters.convertedFdCostParameters = parameters(counter : ...
        counter + length(inputs.numConvertedDiscreteCosts) - 1);
    counter = counter + inputs.numConvertedDiscreteCosts;
end
end
