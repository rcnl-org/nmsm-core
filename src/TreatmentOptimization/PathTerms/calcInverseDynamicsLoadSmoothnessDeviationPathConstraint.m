...
function [pathConstraint, constraintTerm] = ...
    ...
    constraintTerm, inputs, time, inverseDynamicsMoments, loadName)
[idMoment, constraintTerm] = findDataByLabels(constraintTerm, ...
    inverseDynamicsMoments, inputs.inverseDynamicsMomentLabels, ...
    loadName);

polynomialDegree = valueOrAlternate(constraintTerm, ...
    "polynomial_degree", 1);
harmonics = valueOrAlternate(constraintTerm, "harmonics", 7);
fit = fitSmoothingTermCurve(inputs, time, idMoment, ...
    polynomialDegree, harmonics);

pathConstraint = idMoment - fit;

[pathConstraint, constraintTerm] = applyTermTimeRanges(pathConstraint, ...
    constraintTerm, time);
end
