...
experimentalVelocities = findSplinedMarkerVelocitiesByLabels( ...
    constraintTerm, inputs, time);

assert(all(size(velocities, [2, 3]) == [1, 1] & ...
    size(experimentalVelocities, [2, 3]) == [1, 1]), "Marker " + ...
    "constraints may only use one axis.")
pathTerm = velocities - experimentalVelocities;
end
