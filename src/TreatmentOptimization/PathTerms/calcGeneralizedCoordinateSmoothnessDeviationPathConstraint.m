...
function [pathContraint, constraintTerm] = ...
    calcGeneralizedCoordinateSmoothnessDeviationPathConstraint( ...
    constraintTerm, inputs, time, positions, coordinateName)
[position, constraintTerm] = findDataByLabels(constraintTerm, positions, ...
...
fit = fitSmoothingTermCurve(inputs, time, position, ...
    polynomialDegree, harmonics);

pathContraint = position - fit;

[pathContraint, constraintTerm] = applyTermTimeRanges(pathContraint, ...
    constraintTerm, time);
end
