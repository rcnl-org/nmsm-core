...
% smoothed predicted joint accelerations for the specified coordinate.
%
...
function constraint = calcGeneralizedAccelerationSmoothingPathConstraint( ...
    costTerm, inputs, time, accelerations, coordinateName)
...
acceleration = accelerations(:, indx);
...
coefficients = polyFourierCoefs(time, acceleration, frequency, ...
    polynomialDegree, harmonics);
fit = polyFourierCurves(coefficients, frequency, time, ...
    polynomialDegree, 0);
constraint = acceleration - fit;
end
