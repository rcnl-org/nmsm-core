...
% smoothed predicted joint angles for the specified coordinate.
%
...
function constraint = calcGeneralizedCoordinateSmoothingPathConstraint( ...
    costTerm, inputs, time, positions, coordinateName)
...
coefficients = polyFourierCoefs(time, coordinate, frequency, ...
    polynomialDegree, harmonics);
fit = polyFourierCurves(coefficients, frequency, time, ...
    polynomialDegree, 0);
constraint = coordinate - fit;
end
