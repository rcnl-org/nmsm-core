...
function [pathTerm, constraintTerm] = ...
    calcGeneralizedSpeedSymmetryPathConstraint( ...
    inputs, fullVelocities, constraintTerm, values)
assert(length(constraintTerm.coordinates) == 2, ...
    "generalized_speed_symmetry " + ...
    "requires exactly two <coordinates> to compare.")
[velocities, constraintTerm] = findDataByLabels(constraintTerm, ...
    fullVelocities, inputs.coordinateNames, constraintTerm.coordinates);
...
pathTerm = shiftedVelocity - baselineVelocity;
end
