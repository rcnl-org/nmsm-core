...
if (params.tasks{task}.costTerms.rotationError.isEnabled)
    ...
    cost = [cost sqrt(1 / (4 * 101)) * ((1 / maxAllowableError) * ...
        reshape(modeledValues.jointPositions(1:4, :) - ...
        inputs.experimentalJointPositions(1:4, :), 1, [])) .^ 4];
end
if (params.tasks{task}.costTerms.translationError.isEnabled)
    maxAllowableError = ...
        params.tasks{task}.costTerms.translationError.maxAllowableError;
    cost = [cost sqrt(1 / (3 * 101)) * ((1 / maxAllowableError) * ...
        reshape(modeledValues.jointPositions(5:7, :) - ...
        inputs.experimentalJointPositions(5:7, :), 1, [])) .^ 4];
end
