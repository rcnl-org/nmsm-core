...
if isCalculated(1) && isequal(mexext, 'mexw64')
    if valueOrAlternate(inputs, 'calculateBodyLoads', false)
        inputs.surfaces{foot}.footMarkerLocations(end+1 : end+2, :) = ...
            zeros(2, 3);
        inputs.surfaces{foot}.footMarkerBodies(end+1 : end+2) = [0, 1];
        inputs.surfaces{foot}.markerNames.parentBody = 'parent';
        inputs.surfaces{foot}.markerNames.childBody = 'parent';
    end
...
if valueOrAlternate(inputs, 'calculateBodyLoads', false)
    isParentBody = logical(~inputs.surfaces{foot}.springMarkerBodies);
    splitSpringPositions.parent{1} = springPositions(:, :, isParentBody);
    splitSpringPositions.child{1} = springPositions(:, :, ~isParentBody);
    splitSpringVelocities.parent{1} = springVelocities(:, :, isParentBody);
    splitSpringVelocities.child{1} = springVelocities(:, :, ~isParentBody);
    surfaceStruct.contactSurfaces{1}.parentSpringConstants = ...
        values.springConstants(isParentBody);
    surfaceStruct.contactSurfaces{1}.childSpringConstants = ...
        values.springConstants(~isParentBody);
    surfaceStruct.contactSurfaces{1}.dampingFactor = values.dampingFactor;
    surfaceStruct.contactSurfaces{1}.restingSpringLength = ...
        values.restingSpringLength;
    surfaceStruct.contactSurfaces{1}.beltSpeed = ...
        inputs.surfaces{1}.beltSpeed;
    surfaceStruct.contactSurfaces{1}.latchingVelocity = ...
        values.latchingVelocity;
    surfaceStruct.contactSurfaces{1}.dynamicFrictionCoefficient = ...
        values.dynamicFrictionCoefficient;
    surfaceStruct.contactSurfaces{1}.viscousFrictionCoefficient = ...
        values.viscousFrictionCoefficient;
    bodyLocations.midfootSuperior{1} = ...
        modeledValues.markerPositions.midfootSuperior;
    modeledValues.splitGroundReactions = calcFootGroundReactions( ...
        splitSpringPositions, splitSpringVelocities, surfaceStruct, ...
        bodyLocations, false);
end
