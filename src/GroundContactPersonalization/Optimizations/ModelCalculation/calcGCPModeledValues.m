...
if isCalculated(3) || excludeDeflection
...
for i=1:size(modeledJointPositions, 2)
    ...
    if isCalculated(2)
        ...
        for j = 1:length(values.springConstants)
            markerKinematics.height(j) = model.getMarkerSet().get(...
                "spring_marker_" + num2str(j)).getLocationInGround(...
                state).get(1);
            markerKinematics.yVelocity(j) = model.getMarkerSet().get(...
                "spring_marker_" + num2str(j)).getVelocityInGround(...
                state).get(1);
        end
    ...
    if isCalculated(3)
        ...
        for j = 1:length(values.springConstants)
            markerKinematics.xVelocity(j) = model.getMarkerSet().get(...
                "spring_marker_" + num2str(j)).getVelocityInGround(...
                state).get(0);
            markerKinematics.zVelocity(j) = model.getMarkerSet().get(...
                "spring_marker_" + num2str(j)).getVelocityInGround(...
                state).get(2);
        end
    ...
    if isCalculated(4)
        ...
        for j = 1:length(values.springConstants)
            markerKinematics.xPosition(j) = model.getMarkerSet().get(...
                "spring_marker_" + num2str(j)).getLocationInGround(...
                state).get(0);
            markerKinematics.zPosition(j) = model.getMarkerSet().get(...
                "spring_marker_" + num2str(j)).getLocationInGround(...
                state).get(2);
        end
        [modeledValues.xGrfMoment(i), modeledValues.yGrfMoment(i), ...
            ...
            inputs, markerKinematics, springForces);
    end
end
% if isCalculated(1)
%     for i=1:length(markerNamesFields)
