...
if params.tasks{task}.enforcePositiveVerticalForce
    results = lsqnonlin(@(values) calcGroundContactPersonalizationTaskCost( ...
        values, fieldNameOrder, inputs, params, task), initialValues, ...
        lowerBounds, upperBounds, [], [], [], [], @(values) ...
        calcGroundContactPersonalizationNonlinearConstraint( ...
        values, fieldNameOrder, inputs, params, task), optimizerOptions);
else
    results = lsqnonlin(@(values) calcGroundContactPersonalizationTaskCost( ...
        values, fieldNameOrder, inputs, params, task), initialValues, ...
