...
function writeOptimizedFootKinematicsToSto(inputs, resultsDirectory, ...
    modelName)
columnLabels = ["Toe.Angle", "X.Rotation", "Y.Rotation", "Z.Rotation", ...
    "X.Translation", "Y.Translation", "Z.Translation"];
for foot = 1:length(inputs.tasks)
    [modeledJointPositions, ~] = calcGCPJointKinematics( ...
        inputs.tasks{foot}.experimentalJointPositions, ...
        inputs.tasks{foot}.jointKinematicsBSplines, ...
        inputs.tasks{foot}.bSplineCoefficients);
    writeToSto( ...
        columnLabels, ...
        inputs.tasks{foot}.time, ...
