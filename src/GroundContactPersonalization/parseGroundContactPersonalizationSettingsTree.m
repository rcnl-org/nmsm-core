...
stageOneCostTerms = ["markerPositionError", "markerSlopeError", ...
    ...
    "verticalGrfSlopeError", "springConstantErrorFromMean", ...
    "dampingFactorErrorFromMean"];
for i = 1:length(stageOneCostTerms)
    enabled = valueOrAlternate(valueOrAlternate(stageOneTree, ...
        stageOneCostTerms(i), 'none'), 'is_enabled', 'false');
    if (isstruct(enabled)) 
        enabled = enabled.Text;
    end
    params.stageOneCosts.(stageOneCostTerms(i)).isEnabled = strcmpi(...
        enabled, 'true');
    allowableError = valueOrAlternate(valueOrAlternate(stageOneTree, ...
        stageOneCostTerms(i), 'none'), 'max_allowable_error', '1');
    if (isstruct(allowableError))
        allowableError = allowableError.Text;
    end
    params.stageOneCosts.(stageOneCostTerms(i)).maxAllowableError = ...
...
for i = 1:length(stageTwoCostTerms)
    enabled = valueOrAlternate(valueOrAlternate(stageTwoTree, ...
        stageTwoCostTerms(i), 'none'), 'is_enabled', 'false');
    if (isstruct(enabled)) 
        enabled = enabled.Text;
    end
    params.stageTwoCosts.(stageTwoCostTerms(i)).isEnabled = strcmpi(...
        enabled, 'true');
    allowableError = valueOrAlternate(valueOrAlternate(stageTwoTree, ...
        stageTwoCostTerms(i), 'none'), 'max_allowable_error', '1');
    if (isstruct(allowableError))
        allowableError = allowableError.Text;
    end
    params.stageTwoCosts.(stageTwoCostTerms(i)).maxAllowableError = ...
        str2double(allowableError);
end

stageThreeTree = getFieldByNameOrError(tree, 'Stage3CostFunctionTerms');
...
for i = 1:length(stageThreeCostTerms)
    enabled = valueOrAlternate(valueOrAlternate(stageThreeTree, ...
        stageThreeCostTerms(i), 'none'), 'is_enabled', 'false');
    if (isstruct(enabled)) 
        enabled = enabled.Text;
    end
    params.stageThreeCosts.(stageThreeCostTerms(i)).isEnabled = strcmpi(...
        enabled, 'true');
    allowableError = valueOrAlternate(valueOrAlternate(stageThreeTree, ...
        stageThreeCostTerms(i), 'none'), 'max_allowable_error', '1');
    if (isstruct(allowableError))
        allowableError = allowableError.Text;
    end
    params.stageThreeCosts.(stageThreeCostTerms(i)).maxAllowableError = ...
        str2double(allowableError);
end
end