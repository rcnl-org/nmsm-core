...
subplot(2, 3, 1)
plot(inputs.tasks{foot}.time, ...
    inputs.tasks{foot}.experimentalGroundReactionForces(2, :), "red", "LineWidth", 2)
hold on
plot(inputs.tasks{foot}.time, modeledValues.verticalGrf, "blue", "LineWidth", 2)
error = rms(inputs.tasks{foot}.experimentalGroundReactionForces(2, :) - ...
...
for i = 2:6
    ...
    if (i == 2)
        plot(inputs.tasks{foot}.time, ...
            inputs.tasks{foot}.experimentalGroundReactionForces(1, :), "red", ...
            "LineWidth", 2)
        hold on
        plot(inputs.tasks{foot}.time, modeledValues.(groundReactions(i)), ...
            "blue", "LineWidth", 2)
        error = rms(inputs.tasks{foot}.experimentalGroundReactionForces(1, :) - ...
    ...
    elseif (i == 3)
        plot(inputs.tasks{foot}.time, ...
            inputs.tasks{foot}.experimentalGroundReactionForces(i, :), "red", ...
            "LineWidth", 2)
        hold on
        plot(inputs.tasks{foot}.time, modeledValues.(groundReactions(i)), ...
            "blue", "LineWidth", 2)
        error = rms(inputs.tasks{foot}.experimentalGroundReactionForces(i, :) - ...
            modeledValues.(groundReactions(i)));
        title(groundReactions(i) + newline + " RMSE: " + error)
        xlabel('Time')
        hold off
        ...
        plot(inputs.tasks{foot}.time, ...
            inputs.tasks{foot}.experimentalGroundReactionMoments(i - 3, :), ...
            "red", "LineWidth", 2)
        hold on
        plot(inputs.tasks{foot}.time, modeledValues.(groundReactions(i)), ...
            "blue", "LineWidth", 2)
        error = rms(inputs.tasks{foot}.experimentalGroundReactionMoments(i - 3, :) - ...
            modeledValues.(groundReactions(i)));
        title(groundReactions(i) + newline + " RMSE: " + error)
        xlabel('Time')
        if i == 4
