...
%     getFieldByNameOrAlternate(tree, ...
%     'activation_saturation_sharpness', '74')));
...
% To keep runtime low while maintaining performance, set nodes = 26.
end
...
% optimizerOptions in computeNeuralControlOptimization
...
for i = 1 : length(muscleNames)
    if ismember(muscleNames(i), mtpDataMuscleNames)
        ...
        if isfield(currentMuscle, 'optimalFiberLength')
            optimalFiberLengthScaleFactors(i) = ...
                currentMuscle.optimalFiberLength / inputs.optimalFiberLength(i);
        else
            optimalFiberLengthScaleFactors(i) = 1;
        end
        if isfield(currentMuscle, 'tendonSlackLength')
            tendonSlackLengthScaleFactors(i) = ...
                currentMuscle.tendonSlackLength / inputs.tendonSlackLength(i);
        else
