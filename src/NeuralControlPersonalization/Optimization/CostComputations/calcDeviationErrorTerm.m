...
function cost = calcDeviationErrorTerm(value, ...
    errorCenter, weight, maxAllowableError)

value = value(:);
if isempty(errorCenter)
    errorCenter = 0;
else
    errorCenter = errorCenter(:);
end
cost = weight .^ 0.5 * ((value - errorCenter) ./ maxAllowableError) ./ ...
    (numel(value)) .^ 0.5;
cost(isnan(cost)) = 0;
end