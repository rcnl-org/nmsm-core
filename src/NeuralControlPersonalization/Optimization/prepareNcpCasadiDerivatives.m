...
fullValues = expandBilateralSymmetryForCasadi(x, inputs);
objective = calcCasadiNcpCost(fullValues, inputs, params);
objectiveGradient = gradient(objective, x);

useNonlinearConstraint = ~isempty(normalizationTarget);
if useNonlinearConstraint
