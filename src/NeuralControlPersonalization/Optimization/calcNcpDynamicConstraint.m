...
for i = 1 : params.numMeshes
    if isa(commandsControl, 'casadi.MX')
        subtraction = casadi.MX.zeros(size(commandsControl(index + 2 : ...
            index + params.numCollocationPerMesh + 1, :)));
    else
        subtraction = zeros(size(commandsControl(index + 2 : ...
            index + params.numCollocationPerMesh + 1, :)));
    end
    for j = 1 : inputs.numTrials
        subtraction(:, (j - 1) * inputs.numSynergies + 1 : j * ...
            inputs.numSynergies) = commandsControl(index + 2 : ...
        index + params.numCollocationPerMesh + 1, (j - 1) * ...
        inputs.numSynergies + 1 : j * inputs.numSynergies) * ...
        meshDuration(j);
    end
