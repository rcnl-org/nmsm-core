...
degenerate = find(scaleFactors == 0 | ~isfinite(scaleFactors));
if ~isempty(degenerate)
    error(['normalizeSynergiesByMaximumWeight: synergy row(s) [%s] have a ' ...
        'zero or non-finite maximum weight (a degenerate/unused synergy - ' ...
        'often caused by configuring more synergies than the data ' ...
        'supports), which would silently produce NaN/Inf results.'], ...
        num2str(degenerate'));
end
